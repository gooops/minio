...
func doesPolicySignatureV2Match_lp(formValues http.Header) APIErrorCode {
...
func doesPresignV2SignatureMatch_lp(r *http.Request) APIErrorCode {
	// Access credentials.
	cred := serverConfig.GetCredential()
...
func validateV2AuthHeader_lp(v2Auth string) APIErrorCode {
	if v2Auth == "" {
...
func doesSignV2Match_lp(r *http.Request) APIErrorCode {
	v2Auth := r.Header.Get("Authorization")

	if apiError := validateV2AuthHeader_lp(v2Auth); apiError != ErrNone {
		return apiError
	}
	...
	expectedAuth := signatureV2_lp(r.Method, encodedResource, encodedQuery, r.Header)
...
func calculateSignatureV2_lp(stringToSign string, secret string) string {
...
func preSignatureV2_lp(method string, encodedResource string, encodedQuery string, headers http.Header, expires string) string {
	cred := serverConfig.GetCredential()
	stringToSign := presignV2STS(method, encodedResource, encodedQuery, headers, expires)
	return calculateSignatureV2_lp(stringToSign, cred.SecretKey)
}

// Return signature-v2 authrization header.
func signatureV2_lp(method string, encodedResource string, encodedQuery string, headers http.Header) string {
	cred := serverConfig.GetCredential()
	stringToSign := signV2STS(method, encodedResource, encodedQuery, headers)
	signature := calculateSignatureV2_lp(stringToSign, cred.SecretKey)
	return fmt.Sprintf("%s %s:%s", signV2Algorithm, cred.AccessKey, signature)
}

// Return canonical headers.
func canonicalizedAmzHeadersV2_lp(headers http.Header) string {
	var keys []string
...
func canonicalizedResourceV2_lp(encodedPath string, encodedQuery string) string {
	queries := strings.Split(encodedQuery, "&")
	keyval := make(map[string]string)
	for _, query := range queries {
		key := query
	...
	for _, key := range resourceList_lp {
		val, ok := keyval[key]
		if !ok {
			continue
		}
...
func signV2STS_lp(method string, encodedResource string, encodedQuery string, headers http.Header) string {
	canonicalHeaders := canonicalizedAmzHeadersV2_lp(headers)
	...
	stringToSign := strings.Join([]string{
		...
		headers.Get("Date"),
		canonicalHeaders,
	}, "\n") + canonicalizedResourceV2_lp(encodedResource, encodedQuery)

	return stringToSign
}

// Return string to sign for pre-sign signature calculation.
func presignV2STS_lp(method string, encodedResource string, encodedQuery string, headers http.Header, expires string) string {
	canonicalHeaders := canonicalizedAmzHeadersV2_lp(headers)
	if len(canonicalHeaders) > 0 {
		canonicalHeaders += "\n"
	}
