...
func enforceBucketPolicy_lp(bucket, action, resource, referer string, queryParams url.Values) (s3Error APIErrorCode) {
...
func isBucketActionAllowed_lp(action, bucket, prefix string) bool {
	policy := globalBucketPolicies.GetBucketPolicy(bucket)
	if policy == nil {
		return false
	}
...
func (api objectAPIHandlers_lp) GetBucketLocationHandler(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	bucket := vars["bucket"]
	...
	s3Error := checkRequestAuthType_lp(r, bucket, "s3:GetBucketLocation", globalMinioDefaultRegion)
	if s3Error == ErrInvalidRegion {
		// Clients like boto3 send getBucketLocation() call signed with region that is configured.
		s3Error = checkRequestAuthType_lp(r, "", "s3:GetBucketLocation", serverConfig.GetRegion())
	}
	if s3Error != ErrNone {
...
//
func (api objectAPIHandlers_lp) ListMultipartUploadsHandler(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	bucket := vars["bucket"]
	...
	if s3Error := checkRequestAuthType_lp(r, bucket, "s3:ListBucketMultipartUploads", serverConfig.GetRegion()); s3Error != ErrNone {
		writeErrorResponse(w, s3Error, r.URL)
		return
	}
...
func (api objectAPIHandlers_lp) ListBucketsHandler(w http.ResponseWriter, r *http.Request) {
...
func (api objectAPIHandlers_lp) DeleteMultipleObjectsHandler(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	bucket := vars["bucket"]
...
func (api objectAPIHandlers_lp) PutBucketHandler(w http.ResponseWriter, r *http.Request) {
	objectAPI := api.ObjectAPI()
	if objectAPI == nil {
		writeErrorResponse(w, ErrServerNotInitialized, r.URL)
		return
...
func (api objectAPIHandlers_lp) PostPolicyBucketHandler(w http.ResponseWriter, r *http.Request) {
	objectAPI := api.ObjectAPI()
	if objectAPI == nil {
		writeErrorResponse(w, ErrServerNotInitialized, r.URL)
		return
...
func (api objectAPIHandlers_lp) HeadBucketHandler(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	bucket := vars["bucket"]
	...
	if s3Error := checkRequestAuthType_lp(r, bucket, "s3:ListBucket", serverConfig.GetRegion()); s3Error != ErrNone {
...
func (api objectAPIHandlers_lp) DeleteBucketHandler(w http.ResponseWriter, r *http.Request) {
	objectAPI := api.ObjectAPI()
	if objectAPI == nil {
		writeErrorResponse(w, ErrServerNotInitialized, r.URL)
		return
