...
func validateListObjectsArgs_lp(prefix, marker, delimiter string, maxKeys int) APIErrorCode {
...
func (api objectAPIHandlers_lp) ListObjectsV2Handler(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	bucket := vars["bucket"]
...
// criteria to return a subset of the objects in a bucket.
//
func (api objectAPIHandlers_lp) ListObjectsV1Handler(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	bucket := vars["bucket"]
	...
	if s3Error := validateListObjectsArgs_lp(prefix, marker, delimiter, maxKeys); s3Error != ErrNone {
		writeErrorResponse(w, s3Error, r.URL)
		return
	}
