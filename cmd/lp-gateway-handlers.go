...
type gatewayAPIHandlers_lp struct {
	objectAPIHandlers_lp
...
func (api gatewayAPIHandlers_lp) GetObjectHandler(w http.ResponseWriter, r *http.Request) {
	...
	case authTypeSigned_lp, authTypePresigned_lp:
		s3Error := isReqAuthenticated_lp(r, serverConfig.GetRegion())
		if s3Error != ErrNone {
			errorIf(errSignatureMismatch, dumpRequest(r))
			writeErrorResponse(w, s3Error, r.URL)
			return
	...
	if reqAuthType == authTypeAnonymous_lp {
...
func (api gatewayAPIHandlers_lp) PutObjectHandler(w http.ResponseWriter, r *http.Request) {
	objectAPI := api.ObjectAPI()
	if objectAPI == nil {
		writeErrorResponse(w, ErrServerNotInitialized, r.URL)
		return
	...
	reqAuthType := getRequestAuthType_lp(r)
	if reqAuthType == authTypeStreamingSigned_lp {
...
func (api gatewayAPIHandlers_lp) HeadObjectHandler(w http.ResponseWriter, r *http.Request) {
	var object, bucket string
	vars := router.Vars(r)
	bucket = vars["bucket"]
	object = vars["object"]
...
func (api gatewayAPIHandlers_lp) DeleteMultipleObjectsHandler(w http.ResponseWriter, r *http.Request) {
	vars := router.Vars(r)
	bucket := vars["bucket"]
...
func (api gatewayAPIHandlers_lp) PutBucketPolicyHandler(w http.ResponseWriter, r *http.Request) {
	...
	if s3Error := checkRequestAuthType_lp(r, "", "", serverConfig.GetRegion()); s3Error != ErrNone {
		writeErrorResponse(w, s3Error, r.URL)
		return
	}
...
func (api gatewayAPIHandlers_lp) DeleteBucketPolicyHandler(w http.ResponseWriter, r *http.Request) {
	objAPI := api.ObjectAPI()
	if objAPI == nil {
		writeErrorResponse(w, ErrServerNotInitialized, r.URL)
		return
...
func (api gatewayAPIHandlers_lp) GetBucketPolicyHandler(w http.ResponseWriter, r *http.Request) {
	objAPI := api.ObjectAPI()
	if objAPI == nil {
		writeErrorResponse(w, ErrServerNotInitialized, r.URL)
		return
...
func (api gatewayAPIHandlers_lp) GetBucketNotificationHandler(w http.ResponseWriter, r *http.Request) {
...
func (api gatewayAPIHandlers_lp) PutBucketNotificationHandler(w http.ResponseWriter, r *http.Request) {
	writeErrorResponse(w, ErrNotImplemented, r.URL)
}

// ListenBucketNotificationHandler - list bucket notifications.
func (api gatewayAPIHandlers_lp) ListenBucketNotificationHandler(w http.ResponseWriter, r *http.Request) {
	writeErrorResponse(w, ErrNotImplemented, r.URL)
}
...
func (api gatewayAPIHandlers_lp) PutBucketHandler(w http.ResponseWriter, r *http.Request) {
	objectAPI := api.ObjectAPI()
	if objectAPI == nil {
		writeErrorResponse(w, ErrServerNotInitialized, r.URL)
		return
	...
	s3Error := checkRequestAuthType_lp(r, "", "", globalMinioDefaultRegion)
	if s3Error == ErrInvalidRegion {
		// Clients like boto3 send putBucket() call signed with region that is configured.
		s3Error = checkRequestAuthType_lp(r, "", "", serverConfig.GetRegion())
	}
	if s3Error != ErrNone {
		writeErrorResponse(w, s3Error, r.URL)
		return
...
func (api gatewayAPIHandlers_lp) DeleteBucketHandler(w http.ResponseWriter, r *http.Request) {
	objectAPI := api.ObjectAPI()
	if objectAPI == nil {
		writeErrorResponse(w, ErrServerNotInitialized, r.URL)
		return
...
//
func (api gatewayAPIHandlers_lp) ListObjectsV1Handler(w http.ResponseWriter, r *http.Request) {
	vars := router.Vars(r)
	bucket := vars["bucket"]
...
func (api gatewayAPIHandlers_lp) HeadBucketHandler(w http.ResponseWriter, r *http.Request) {
	vars := router.Vars(r)
	bucket := vars["bucket"]
...
func (api gatewayAPIHandlers_lp) GetBucketLocationHandler(w http.ResponseWriter, r *http.Request) {
	vars := router.Vars(r)
	bucket := vars["bucket"]
	...
	case authTypeSigned_lp, authTypePresigned_lp:
		s3Error := isReqAuthenticated_lp(r, globalMinioDefaultRegion)
		if s3Error == ErrInvalidRegion {
