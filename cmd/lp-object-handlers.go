...
type objectAPIHandlers_lp struct {
	ObjectAPI func() ObjectLayer
}

// supportedGetReqParams_lp - supported request parameters for GET presigned request.
var supportedGetReqParams_lp = map[string]string{
...
// setGetRespHeaders_lp - set any requested parameters as response headers.
func setGetRespHeaders_lp(w http.ResponseWriter, reqParams url.Values) {
	for k, v := range reqParams {
		if header, ok := supportedGetReqParams_lp[k]; ok {
			w.Header()[header] = v
		}
	}
}
...
func errAllowableObjectNotFound_lp(bucket string, r *http.Request) APIErrorCode {
	if getRequestAuthType_lp(r) == authTypeAnonymous_lp {
...
type funcToWriter_lp func([]byte) (int, error)

func (f funcToWriter_lp) Write(p []byte) (int, error) {
	return f(p)
}
...
func (api objectAPIHandlers_lp) GetObjectHandler(w http.ResponseWriter, r *http.Request) {
	...
	if s3Error := checkRequestAuthType_lp(r, bucket, "s3:GetObject", serverConfig.GetRegion()); s3Error != ErrNone {
	...
	if err != nil {
		...
		if apiErr == ErrNoSuchKey {
			apiErr = errAllowableObjectNotFound_lp(bucket, r)
		}
	...
	writer := funcToWriter_lp(func(p []byte) (int, error) {
		if !dataWritten {
			...
			setGetRespHeaders_lp(w, r.URL.Query())
...
func (api objectAPIHandlers_lp) HeadObjectHandler(w http.ResponseWriter, r *http.Request) {
	var object, bucket string
	vars := mux.Vars(r)
	bucket = vars["bucket"]
	object = vars["object"]
...
func getCpObjMetadataFromHeader_lp(header http.Header, defaultMeta map[string]string) (map[string]string, error) {
...
func (api objectAPIHandlers_lp) CopyObjectHandler(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	...
	if s3Error := checkRequestAuthType_lp(r, dstBucket, "s3:PutObject", serverConfig.GetRegion()); s3Error != ErrNone {
		writeErrorResponse(w, s3Error, r.URL)
		return
	}
	...
	newMetadata, err := getCpObjMetadataFromHeader_lp(r.Header, defaultMeta)
	if err != nil {
...
// ----------
// This implementation of the PUT operation adds an object to a bucket.
func (api objectAPIHandlers_lp) PutObjectHandler(w http.ResponseWriter, r *http.Request) {
	objectAPI := api.ObjectAPI()
	if objectAPI == nil {
		writeErrorResponse(w, ErrServerNotInitialized, r.URL)
		return
	...
	rAuthType := getRequestAuthType_lp(r)
	if rAuthType == authTypeStreamingSigned_lp {
	...
	case authTypeStreamingSigned_lp:
	...
	case authTypeSignedV2_lp, authTypePresignedV2_lp:
		...
		objInfo, err = objectAPI.PutObject(bucket, object, size, r.Body, metadata, sha256sum)
	case authTypePresigned_lp, authTypeSigned_lp:
		if s3Error := reqSignatureV4Verify_lp(r, serverConfig.GetRegion()); s3Error != ErrNone {
			errorIf(errSignatureMismatch, dumpRequest(r))
			writeErrorResponse(w, s3Error, r.URL)
			return
		}
...
/// Multipart objectAPIHandlers_lp

// NewMultipartUploadHandler - New multipart upload.
func (api objectAPIHandlers_lp) NewMultipartUploadHandler(w http.ResponseWriter, r *http.Request) {
	var object, bucket string
	vars := mux.Vars(r)
	bucket = vars["bucket"]
	object = vars["object"]
	...
	if s3Error := checkRequestAuthType_lp(r, bucket, "s3:PutObject", serverConfig.GetRegion()); s3Error != ErrNone {
		writeErrorResponse(w, s3Error, r.URL)
		return
	}
...
func (api objectAPIHandlers_lp) CopyObjectPartHandler(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	dstBucket := vars["bucket"]
	dstObject := vars["object"]
...
func (api objectAPIHandlers_lp) PutObjectPartHandler(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	bucket := vars["bucket"]
	object := vars["object"]
...
func (api objectAPIHandlers_lp) AbortMultipartUploadHandler(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	bucket := vars["bucket"]
	object := vars["object"]
	...
	if s3Error := checkRequestAuthType_lp(r, bucket, "s3:AbortMultipartUpload", serverConfig.GetRegion()); s3Error != ErrNone {
		writeErrorResponse(w, s3Error, r.URL)
		return
	}
...
func (api objectAPIHandlers_lp) ListObjectPartsHandler(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	bucket := vars["bucket"]
	object := vars["object"]
	...
	if s3Error := checkRequestAuthType_lp(r, bucket, "s3:ListMultipartUploadParts", serverConfig.GetRegion()); s3Error != ErrNone {
		writeErrorResponse(w, s3Error, r.URL)
		return
	}
...
func (api objectAPIHandlers_lp) CompleteMultipartUploadHandler(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	bucket := vars["bucket"]
	object := vars["object"]
...
/// Delete objectAPIHandlers_lp

// DeleteObjectHandler - delete an object
func (api objectAPIHandlers_lp) DeleteObjectHandler(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	bucket := vars["bucket"]
	object := vars["object"]
	...
	if s3Error := checkRequestAuthType_lp(r, bucket, "s3:DeleteObject", serverConfig.GetRegion()); s3Error != ErrNone {
		writeErrorResponse(w, s3Error, r.URL)
		return
	}
