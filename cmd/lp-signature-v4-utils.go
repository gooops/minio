...
const unsignedPayload_lp = "UNSIGNED-PAYLOAD"
...
func isValidRegion_lp(reqRegion string, confRegion string) bool {
	if confRegion == "" {
		return true
	}
	if confRegion == "US" {
...
func sumHMAC_lp(key []byte, data []byte) []byte {
...
var reservedNames_lp = regexp.MustCompile("^[a-zA-Z0-9-_.~/]+$")
...
func getURLEncodedName_lp(name string) string {
	// if object matches reserved string, no need to encode them
	if reservedNames_lp.MatchString(name) {
		return name
	}
...
func extractSignedHeaders_lp(signedHeaders []string, r *http.Request) (http.Header, APIErrorCode) {
	reqHeaders := r.Header
...
func signV4TrimAll_lp(input string) string {
